...
#You will also be able to divide the check evenly according to the number of people.

sub_amount = float(input("How much was your meal? "))
service = str(input("How was your service? Pick one: Good, Fair, or Bad. "))
num_people = int(input("How would you like to split the check? "))
...
split_Good = service_Good / num_people
split_Fair = service_Fair / num_people
split_Bad = service_Bad / num_people
...
if service.upper() == "GOOD":
    ...
    print (("The total amount per person is: %.02f.") % (split_Good))
    ...
    print (("The total amount per person is: %.02f.") % (split_Fair))
    ...
    print (("The total amount per person is: %.02f.") % (split_Bad))
else:
